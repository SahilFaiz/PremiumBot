...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, LOG_CHANNEL, SPELL_IMG, \
    MAX_B_TN, \
    NO_RESULTS_MSG, LANGUAGES, SEASONS, PREMIUM_USER
...
async def give_filter(client, message):
    if not await forcedjoined(client, message):
        return
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
        text=f"<b>𝗛𝗲𝘆 {user} 😍 ,\n\n𝗬𝗮𝗵𝗮 𝗔𝗮𝗽 𝗠𝗼𝘃𝗶𝗲𝘀 𝗦𝗲𝗮𝗿𝗰𝗵 𝗻𝗵𝗶 𝗸𝗿 𝘀𝗮𝗸𝘁𝗲 𝗵𝗼. 𝗚𝗿𝗼𝘂𝗽 𝗺𝗲𝗶 𝗷𝗮𝗮𝗸𝗿 𝗠𝗼𝘃𝗶𝗲𝘀 𝗼𝗿 𝗪𝗲𝗯𝘀𝗲𝗿𝗶𝗲𝘀 𝗡𝗮𝗺𝗲 𝗧𝘆𝗽𝗲 𝗸𝗿𝗼 <a href=https://t.me/FilmymodMovies>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ 👇</a></b>",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/FilmymodMovies")]]))
...
async def next_page(bot, query):
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"),
                            InlineKeyboardButton(f"{math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}",
                                                 callback_data="pages"),
    ...
    time_difference = timedelta(hours=cur_time.hour, minutes=cur_time.minute,
                                seconds=(cur_time.second + (cur_time.microsecond / 1000000))) - timedelta(
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                ...
                btn.append(
                    [InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"),
                     InlineKeyboardButton(text=f"1/{math.ceil(int(total_results) / int(MAX_B_TN))}",
                                          callback_data="pages"),
                     InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    search = search.replace("_", " ")
    sea = ""
    season_search = ["s01", "s02", "s03", "s04", "s05", "s06", "s07", "s08", "s09", "s10", "season 01", "season 02",
                     "season 03", "season 04", "season 05", "season 06", "season 07", "season 08", "season 09",
                     "season 10", "season 1", "season 2", "season 3", "season 4", "season 5", "season 6", "season 7",
                     "season 8", "season 9"]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
                ...
                if clicked == typed:
                    await client.send_cached_media(
                        ...
                        reply_markup=InlineKeyboardMarkup(
                            [
                                [
                                    InlineKeyboardButton('🫨ꜱᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url='https://t.me/+oncQpg01Q7gyNzE1'),
                                    InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url='https://t.me/Movies7x')
                                ], [
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset),
                                                          filter=True)
        await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name,
                       query)
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    InlineKeyboardButton('Fɪʟᴇ Sᴇɴᴅ Mᴏᴅᴇ',
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user),
                                          text=f"<b>Hᴇʏ {user.mention}, Sᴏʀʀʏ Yᴏᴜʀ ʀᴇᴏ̨ᴜᴇsᴛ ɪs ᴜɴᴀᴠᴀɪʟᴀʙʟᴇ. Sᴏ ᴏᴜʀ ᴍᴏᴅᴇʀᴀᴛᴏʀs ᴄᴀɴ'ᴛ ᴜᴘʟᴏᴀᴅ ɪᴛ.</b>",
                                          reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID),
        ...
        btn2 = [[
            ...
            InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/FilmymodMovies")
        ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user),
                                          text=f"<b>Hᴇʏ {user.mention}, Yᴏᴜʀ <a ʀᴇᴏ̨ᴜᴇsᴛ ɪs ᴀʟʀᴇᴀᴅʏ ᴀᴠᴀɪʟᴀʙʟᴇ ᴏɴ ᴏᴜʀ ʙᴏᴛ's ᴅᴀᴛᴀʙᴀsᴇ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.</b>",
                                          reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID),
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🍂 ᴜᴘᴅᴀᴛᴇ🍂', url='https://t.me/Filmymod123'),
            InlineKeyboardButton('🫨 ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ', url='https://t.me/FilmymodMovies')
        ], [
...
async def advantage_spell_chok(client, msg):
    ...
    if not g_s:
        ...
        reply_markup = InlineKeyboardMarkup([[
            InlineKeyboardButton("🔍 𝐂𝐥𝐢𝐜𝐤 𝐓𝐨 𝐂𝐡𝐞𝐜𝐤 𝐒𝐩𝐞𝐥𝐥𝐢𝐧𝐠 ✅",
                                 url=f"https://www.google.com/search?q={reqst_gle}+movie")
        ], [
            InlineKeyboardButton("🔍 𝐂𝐥𝐢𝐜𝐤 𝐓𝐨 𝐂𝐡𝐞𝐜𝐤 𝐑𝐞𝐥𝐞𝐚𝐬𝐞 𝐃𝐚𝐭𝐞 📅",
                                 url=f"https://www.google.com/search?q={reqst_gle}+release+date")
        ]]
        )
        if NO_RESULTS_MSG:
